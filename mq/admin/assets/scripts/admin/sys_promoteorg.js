		...
		var grid = new Datatable();
            grid.init({
                ...
                dataTable: {  // here you can define a typical datatable settings from http://datatables.net/usage/options 
                    ...
                    "sAjaxSource": "../index.php/Admin/Admin/findOrgByPromote", // ajax source
                    "bSort":false,//不允许排序
                    "aaSorting": [[ 1, "asc" ]], // set first column as a default sort by asc
                    "fnServerParams" : function (aoData) {
					aoData.push(
						{ "name": "userid_search", "value": $("#userid_search").val() },
						{ "name": "orgnumber_search", "value": $("#orgnumber_search").val() },
...
function openOrgPromote(pkid) {
	var util = new Util();
	var openmodal = $("#ajax-orgreview");
	$("#review_data").data("pkid", pkid);
	util.showLoading();
	openmodal.load('sys_promoteorgdetail.html', '', function() {
		util.getUrl("/Admin/Admin/findOrgPromoteByPkid/pkid/" + pkid, function(data, status) {
			try {
				var objdata = JSON.parse(data);
				var sortno = objdata[0].sortno;
				var name = objdata[0].orgname;
				var userid = objdata[0].userid;
				var identitynumber = objdata[0].orgnumber;
				...
				$('#orgnumber_tpdetail').html(identitynumber);
				$('#orgname_tpdetail').html(name + '(ID:'+userid+')');
				$('#status_tpdetail').html(status);
				$('#authpro_tpdetail').html(authpro);
			} catch (err) {
				util.errorMsg('找不到机构信息');
			} finally {
				util.hideLoading();
...
function doReview() {
	...
	var util = new Util();
	util.showLoading();
	util.postUrl('/Admin/Admin/revieworgpromote/lessonid/' + pkid, function(data, status) {
		if (data == "1") {
			$("#ajax-orgreview").modal('hide');
			ProductAdd.init("../index.php/Admin/Admin/findOrgByPromote");
		}
		util.hideLoading();
	},
	objdata,
