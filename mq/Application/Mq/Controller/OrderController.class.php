...
class OrderController extends Controller {
	...
	public function findOrderByStatus($userid, $status) {
		...
		if ($status == 'going') {
			$statussql = " and (m.status=0 or m.status=1 or m.status=2 or m.status=6 or m.status=7)";
			$statussqlcount = " and (status=0 or status=1 or status=2 or status=6 or status=7)";
		} else if ($status == 'finished') {
			$statussql = " and (m.status=3 or m.status=4 or m.status=5  or m.status=8)";
			$statussqlcount = " and (status=3 or status=4 or status=5 or status=8)";
		}
