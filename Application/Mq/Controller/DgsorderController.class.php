...
//			$datamain['sendtime'] = date("Y-m-d H:i", $datamain['sendtime']);
	...
	public function findProductOrderByStatusAndUserid($status) {
		$query = new \Think\Model();
		$condition_sql = "";
		$count_sql = "";
		$query_sql = "";
...
//			}
	...
	public function cancle($bid){
		...
		$dgs_dao = M("Orderdgs");
		$data_dgs['cancletime'] = time();
		$data_dgs['cancleopt'] = session('userid');
		$data_dgs['cancleoptname'] = session("name");
		$dgs_dao->where("orderid='$bid'")->save($data_dgs);
		echo "yes";
	}
