function openProvider(pid){
	var util = new Util();
    var openmodal = $("#ajax-modal");
    util.showLoading();
    openmodal.load('admin_provider_certinfo.html',function(){
    		util.postUrl('/Admin/Admin/getProviderInfo/pid/'+pid,function(data,status){
...
function verify(pid,pname){
	$("#v_pname").html(pname);
	$("#agent_order_data").data("pid",pid);
	$("#send_confirm").modal('show');
}
$(document).ready(function(){	
	$("#confirm_send_btn").bind("click",function(){
		var pid = $("#agent_order_data").data("pid");
		util.postUrl('/Admin/Admin/authProvider/pid/'+pid,function(data,status){
			var util = new Util();
			if(data=="yes"){
				ProviderOrder.init("../index.php/Admin/Admin/getProvider");
