...
function doDelWorker() {
	...
	var util = new Util();
	util.showLoading();
	util.postUrl('/Mq/CW/delcwphp/pkid/' + pkid, function(data, status) {
			if(data == "yes") {
			...
			ProviderOrder.init("../index.php/Mq/CW/findCwphp", index);
			util.hideLoading();
		},
		objdata,
...
function openEdit(pid) {
	document.location.href = "cw_php_edit.php?tag=cwadmin&item=1&pkid="+pid;
}

function openView(pid) {
	document.location.href = "cw_php_edit.php?tag=cwadmin&item=1&type=display&pkid="+pid;
}
...
//		});
	...
	if(util.isNullStr(params)) {
		ProviderOrder.init("../index.php/Mq/CW/findCwphp", start);
	} else {
