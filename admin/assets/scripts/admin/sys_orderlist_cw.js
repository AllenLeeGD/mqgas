...
function doHe() {
	...
	var objdata = {};
	objdata.hemsg = $("#hemsg").val();
	var util = new Util();
	if(util.isNullStr(objdata.hemsg)){
		util.errorMsg('请填写核实信息');
		return;
	}
	util.showLoading();
	util.postUrl('/Mq/JMOrder/he/bid/' + pkid, function(data, status) {
			if(data == "yes") {
				util.successMsg('核款成功');
				$("#do_he").modal('hide');
			} else {
				util.errorMsg('核款失败');
			}
			ProviderOrder.init("../index.php/Mq/JMOrder/findProductOrderByStatus/status/6", 0);
			util.hideLoading();
		},
		objdata,
...
$(document).ready(function() {
	...
	if(util.isNullStr(params)) {
		...
		ProviderOrder.init("../index.php/Mq/JMOrder/findProductOrderByStatus/status/6", start);
	}
	
	$("#he_tab").bind('click', function() {
