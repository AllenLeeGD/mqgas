		...
		var grid = new Datatable();
            grid.init({
                ...
                dataTable: {  // here you can define a typical datatable settings from http://datatables.net/usage/options 
                    ...
                    "sAjaxSource": "../index.php/Admin/Admin/findTutorByPromote", // ajax source
                    "bSort":false,//不允许排序
                    "aaSorting": [[ 1, "asc" ]], // set first column as a default sort by asc
                    "fnServerParams" : function (aoData) {
					aoData.push(
						{ "name": "userid_search", "value": $("#userid_search").val() },
						{ "name": "identitynumber_search", "value": $("#identitynumber_search").val() },
...
function openTutorPromote(pkid) {
	var util = new Util();
	var openmodal = $("#ajax-tutorreview");
	$("#review_data").data("pkid", pkid);
	util.showLoading();
	openmodal.load('sys_promotetutordetail.html', '', function() {
		util.getUrl("/Admin/Admin/findTutorPromoteByPkid/pkid/" + pkid, function(data, status) {
			try {
				var objdata = JSON.parse(data);
				var sortno = objdata[0].sortno;
				var name = objdata[0].name;
				var userid = objdata[0].userid;
				var identitynumber = objdata[0].indentitynumber;
				var ispromote = objdata[0].ispromote;
				var status = objdata[0].status;
				var authxueli = objdata[0].authxueli;
				var authzige = objdata[0].authzige;
				var authpro = objdata[0].authpro;
				if(status==1){
					status = '已认证';
				}else{
					status = '未认证';
				...
				$('#identitynumber_tpdetail').html(identitynumber);
				$('#name_tpdetail').html(name + '(ID:'+userid+')');
				$('#status_tpdetail').html(status);
				$('#authxueli_tpdetail').html(authxueli);
				$('#authzige_tpdetail').html(authzige);
				$('#authpro_tpdetail').html(authpro);
			} catch (err) {
				util.errorMsg('找不到家教信息');
			} finally {
				util.hideLoading();
...
function doReview() {
	...
	var util = new Util();
	util.showLoading();
	util.postUrl('/Admin/Admin/reviewtutorpromote/lessonid/' + pkid, function(data, status) {
		if (data == "1") {
			$("#ajax-tutorreview").modal('hide');
			ProductAdd.init("../index.php/Admin/Admin/findTutorByPromote");
		}
		util.hideLoading();
	},
	objdata,
