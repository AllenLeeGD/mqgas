...
function doDelWorker() {
	...
	util.showLoading();
	util.postUrl('/Mq/CW/delcwmtsj/pkid/' + pkid, function(data, status) {
			if(data == "yes") {
			...
			ProviderOrder.init("../index.php/Mq/CW/findCwmtsj/type/"+type, index);
			util.hideLoading();
		},
		objdata,
...
function openEdit(pid) {
	...
	document.location.href = "cw_mtsj_edit.php?tag=cwadmin&item="+t+"&pkid="+pid+"&type="+type;
}
...
function openView(pid) {
	...
	document.location.href = "cw_mtsj_edit.php?tag=cwadmin&item="+t+"&showtype=display&pkid="+pid+"&type="+type;
}
		...
		grid.init({
			...
			dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
				...
				"fnServerParams": function(aoData) {
					aoData.push({
						"name": "bname_search",
						"value": $("#bname_search").val()
					},{
						"name": "snumber_search",
						"value": $("#snumber_search").val()
					},{
						"name": "lname_search",
						"value": $("#lname_search").val()
					});
				}
			}
		});
...
//		});
	...
	if(type==0){
		...
		$("#stitle").html("增加、编辑、删除码头收据(气款)信息");
		$("#atitle").html("新增码头收据(气款)信息");
		$("#atitle").attr("href","cw_mtsj_add.php?tag=cwadmin&item=2&type="+type);
		...
		$("#stitle").html("增加、编辑、删除码头收据(押金)信息");
		$("#atitle").html("新增码头收据(押金)信息");
		$("#atitle").attr("href","cw_mtsj_add.php?tag=cwadmin&item=3&type="+type);
		$("#ltitle").html("财务码头收据(押金)管理");
		$("#ltitle").attr("href","cw_mtsj.php?tag=cwadmin&item=3");
	}
	if(util.isNullStr(params)) {
		ProviderOrder.init("../index.php/Mq/CW/findCwmtsj/type/"+type, start);
	} else {
		...
		$('#bname_search').val(arrval0[1]);
		$('#snumber_search').val(arrval1[1]);
		$('#lname_search').val(arrval2[1]);
		ProviderOrder.init("../index.php/Mq/CW/findCwmtsj/type/"+type, start);
	}
