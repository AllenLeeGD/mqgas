...
function doDelWorker() {
	...
	var util = new Util();
	util.showLoading();
	util.postUrl('/Mq/Check/delrecall/pkid/' + pkid, function(data, status) {
			if(data == "yes") {
...
function openEdit(pid) {
	document.location.href = "sys_recall_edit.php?tag=sysadmin&item=13&pkid="+pid;
}
...
$(document).ready(function() {
	...
	$("#addBtn").attr("href","sys_recall_add.php?tag=sysadmin&item=13&memberid="+memberid);
	if(util.isNullStr(start)) {
		start = 0;
	}
	if(util.isNullStr(params)) {
		ProviderOrder.init("../index.php/Mq/Check/findRecall/memberid/"+memberid, start);
	} else {
