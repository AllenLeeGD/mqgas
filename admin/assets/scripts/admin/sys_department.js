...
function doDelWorker() {
	...
	var util = new Util();
	util.showLoading();
	util.postUrl('/Mq/Role/deldepartmentdetail/pkid/' + pkid, function(data, status) {
			if(data == "yes") {
			...
			ProviderOrder.init("../index.php/Mq/Role/findDepartment", index);
			util.hideLoading();
		},
		objdata,
...
function openEdit(pid) {
	document.location.href = "sys_department_edit.php?tag=sysadmin&item=5&pkid="+pid;
}
...
$(document).ready(function() {
	...
	if(util.isNullStr(params)) {
		ProviderOrder.init("../index.php/Mq/Role/findDepartment", start);
	} else {
