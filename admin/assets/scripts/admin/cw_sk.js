...
function doDelWorker() {
	...
	var util = new Util();
	util.showLoading();
	util.postUrl('/Mq/CW/delcwsk/pkid/' + pkid, function(data, status) {
			if(data == "yes") {
			...
			ProviderOrder.init("../index.php/Mq/CW/findCwsk", index);
			util.hideLoading();
		},
		objdata,
...
function openEdit(pid) {
	document.location.href = "cw_sk_edit.php?tag=cwadmin&item=0&pkid="+pid;
}

function openView(pid) {
	document.location.href = "cw_sk_edit.php?tag=cwadmin&item=0&type=display&pkid="+pid;
}
...
//		});
	...
	if(util.isNullStr(params)) {
		ProviderOrder.init("../index.php/Mq/CW/findCwsk", start);
	} else {
		params = base64_decode(params);
		var arrparam = params.split(',');
		var arrval0 = arrparam[0].split(':');
		$('#cname_search').val(arrval0[1]);
		ProviderOrder.init("../index.php/Mq/CW/findCwsk", start);
	}
