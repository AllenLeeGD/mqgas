...
function doCancle() {
	...
	var util = new Util();
	util.showLoading();
	util.postUrl('/Mq/JMOrder/cancle/bid/' + pkid, function(data, status) {
			if(data == "yes") {
			...
			ProviderOrder.init("../index.php/Mq/JMOrder/findProductOrderByStatus/status/0", 0);
			util.hideLoading();
		},
		objdata,
...
function doSend() {
	...
	var util = new Util();
	util.showLoading();
	util.postUrl('/Mq/JMOrder/send/bid/' + pkid, function(data, status) {
			if(data == "yes") {
				util.successMsg('发送订单成功');
				$("#do_send").modal('hide');
			} else {
				util.errorMsg('发送订单失败');
			}
			ProviderOrder.init("../index.php/Mq/JMOrder/findProductOrderByStatus/status/0", 0);
			util.hideLoading();
		},
...
function doFen() {
	...
	objdata.did = $("#did").val();
	objdata.pid = $("#pid").val();
	objdata.dname = $("#did").find("option:selected").text();
	objdata.pname = $("#pid").find("option:selected").text();
	if(util.isNullStr(objdata.did) || util.isNullStr(objdata.pid)){
		util.errorMsg('请选择门店和片区');
		return;
	}
	util.showLoading();
	util.postUrl('/Mq/JMOrder/dofen/bid/' + pkid, function(data, status) {
			if(data == "yes") {
				util.successMsg('分派成功');
				$("#do_fen").modal('hide');
			} else {
				util.errorMsg('分派失败');
			}
...
function loadDepartment() {
	...
	util.postUrl(
		...
		function(data, status) { //如果调用php成功  
			send_vue = new Vue({
				el:"#form_app",
				data:{sendobj:{},departments:data,pianqus:[]},
				methods:{
					getpianqu:function(){
						var did = $("#did").val();
...
$(document).ready(function() {
	...
	if(util.isNullStr(params)) {
		...
		ProviderOrder.init("../index.php/Mq/JMOrder/findProductOrderByStatus/status/0", start);
	}
	//待出库订单
	$("#zan_tab").bind('click', function() {
