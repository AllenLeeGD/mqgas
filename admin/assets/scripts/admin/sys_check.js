...
function doDelWorker() {
	...
	var util = new Util();
	util.showLoading();
	util.postUrl('/Mq/Check/delcheck/pkid/' + pkid, function(data, status) {
			if(data == "yes") {
...
function openEdit(pid) {
	document.location.href = "sys_check_edit.php?tag=sysadmin&item=13&pkid="+pid;
}
		...
		grid.init({
			...
			dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
				...
				"fnServerParams": function(aoData) {
					aoData.push({
						"name": "checkdate_search",
						"value": $("#checkdate_search").val()
					});
				}
			}
		});
...
$(document).ready(function() {
	...
	$("#addBtn").attr("href","sys_check_add.php?tag=sysadmin&item=13&memberid="+memberid);
	if(util.isNullStr(start)) {
		start = 0;
	}
	if(util.isNullStr(params)) {
		ProviderOrder.init("../index.php/Mq/Check/findCheck/memberid/"+memberid, start);
	} else {
